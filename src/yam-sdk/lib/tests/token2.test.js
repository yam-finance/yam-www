  ...
  describe("expected fail", () => {
    test("cant transfer from a 0 balance", async () => {
      await yam.testing.expectThrow(
        yam.contracts.yamV2.methods.transfer(user, "100").send({ from: new_user }),
        "ERC20: transfer amount exceeds balance"
      );
    });
    test("cant transferFrom without allowance", async () => {
      await yam.contracts.yam.methods
        .approve(yam.contracts.yamV2migration.options.address, "10000000000000000000000000000000000")
        .send({ from: user, gas: 1000000 });
      await yam.contracts.yamV2migration.methods.migrate().send({ from: user });
      await yam.testing.expectThrow(
        yam.contracts.yamV2.methods.transferFrom(user, new_user, "100").send({ from: new_user }),
    ...
    test("decreaseAllowance from 0", async () => {
      ...
      await yam.contracts.yamV2migration.methods.migrate().send({ from: user });
      await yam.testing.expectThrow(
        yam.contracts.yamV2.methods.decreaseAllowance(new_user, "100").send({ from: user }),
