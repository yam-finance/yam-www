...
describe("YAM governance", () => {
  ...
  describe("delegateBySig", () => {
    ...
    test("reverts if the signatory is invalid", async () => {
      ...
      await yam.testing.expectThrow(
        yam.contracts.yam.methods.delegateBySig(delegatee, nonce, expiry, 0, "0xbad", "0xbad").send({ from: a1 }),
        "YAM::delegateBySig: invalid signature"
      );
    });
    ...
    test("reverts if the nonce is bad ", async () => {
      ...
      await yam.testing.expectThrow(
        yam.contracts.yam.methods.delegateBySig(user, 1, 0, sig.v, sig.r, sig.s).send({ from: a1 }),
        "YAM::delegateBySig: invalid nonce"
      );
    });
    ...
    test("reverts if the signature has expired", async () => {
      ...
      await yam.testing.expectThrow(
        yam.contracts.yam.methods.delegateBySig(user, 0, 0, sig.v, sig.r, sig.s).send({ from: a1 }),
        "YAM::delegateBySig: signature expired"
      );
    });
  ...
  describe("numCheckpoints", () => {
    it("returns the number of checkpoints for a delegate", async () => {
      ...
      await yam.contracts.yam.methods.transfer(a2, one_hundred.minus(one_hundred.times(0.9)).toString()).send({ from: guy });
      nc = await yam.contracts.yam.methods.numCheckpoints(a1).call();
      expect(nc).toBe("2");
      ...
      await yam.contracts.yam.methods.transfer(guy, one_hundred.minus(one_hundred.times(0.8)).toString()).send({ from: user });
      nc = await yam.contracts.yam.methods.numCheckpoints(a1).call();
      expect(nc).toBe("4");
    ...
    it("does not add more than one checkpoint in a block", async () => {
      ...
      let t2 = yam.contracts.yam.methods
        .transfer(
          a2,
          yam
            .toBigN(100)
        ...
        .send({ from: guy });
      let t3 = yam.contracts.yam.methods
        .transfer(
          a2,
          yam
            .toBigN(100)
      ...
      let t4 = await yam.contracts.yam.methods
        .transfer(
          guy,
          yam
            .toBigN(100)
  ...
  describe("getPriorVotes", () => {
    test("reverts if block number >= current block", async () => {
      await yam.testing.expectThrow(yam.contracts.yam.methods.getPriorVotes(a1, 5e10).call(), "YAM::getPriorVotes: not yet determined");
    });
    ...
    it("generally returns the voting balance at the appropriate checkpoint", async () => {
      ...
      await yam.testing.mineBlock();
      await yam.testing.mineBlock();
      let t2 = await yam.contracts.yam.methods
        .transfer(
          a2,
          yam
            .toBigN(100)
      ...
      await yam.testing.mineBlock();
      await yam.testing.mineBlock();
      let t3 = await yam.contracts.yam.methods
        .transfer(
          a2,
          yam
            .toBigN(100)
