...
describe("rebase_tests", () => {
  ...
  describe("rebase", () => {
    ...
    test("init_twap", async () => {
      ...
      await yam.contracts.uni_router.methods
        .swapExactTokensForTokens(1000, 100, [yam.contracts.yam.options.address, yam.contracts.ycrv.options.address], user, 1596740361 + 10000000)
        .send({
          from: user,
          gas: 1000000,
        });
    ...
    test("positive rebasing", async () => {
      ...
      let res_bal = await yam.contracts.yam.methods.balanceOf(yam.contracts.reserves.options.address).call();
  ...
  describe("failing", () => {
    test("unitialized rebasing", async () => {
      await yam.testing.expectThrow(
        yam.contracts.rebaser.methods.activate_rebasing().send({
          from: user,
          gas: 500000,
        }),
    ...
    test("no early twap", async () => {
      await yam.testing.expectThrow(
        yam.contracts.rebaser.methods.init_twap().send({
          from: user,
          gas: 500000,
        }),
        ""
    ...
    test("too late rebasing", async () => {
      ...
      let b = await yam.testing.expectThrow(
        yam.contracts.rebaser.methods.rebase().send({
          from: user,
          gas: 2500000,
        }),
        "too late"
