...
const MigrateCard: React.FC = () => {
  ...
  const { isApproved, isApproving, isMigrating, onApprove, onMigrate } = useMigration();
  ...
  const ActionButton = useMemo(() => {
    ...
    if (isMigrating) {
      return <Button disabled full text="Migrating..." variant="secondary" />;
    }
    if (isApproved) {
      return <Button disabled={!hasYams} full onClick={onMigrate} text="Migrate" variant={hasYams ? "default" : "secondary"} />;
    }
    return (
      <Button
  ...
  }, [status, isApproved, isApproving, isMigrating, onApprove, onMigrate, yamV2Balance]);
